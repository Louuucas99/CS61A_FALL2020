...
#这段不知道ok为啥不过，自己试的都没什么问题，我觉得写的挺好的
#后来试了几个网上的答案，发现一样报
#Test summary：3 test cases passed before encountering first failed test case
#弃疗，就用爸爸这个了
    def say(score0,score1):
        nonlocal last_score,running_high  #不加报错UnboundLocalError: local variable 'last_score' referenced before assignment
        incr = (score0 if who == 0 else score1) - last_score
        if incr > running_high:
            print(incr,'point(s)! The most yet for Player',who)
            running_high = incr
        last_score += incr
